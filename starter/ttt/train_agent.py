...
class Environment:
    ...
    def perform(self, action, player):
        ...
        reward_player2 = LOST_REWARD if win else DRAW_REWARD if draw else 0
...
class SmartBotModel:
    def train(self, verbose=False):
        ...
        ALPHAS = np.linspace(MAX_ALPHA, MIN_ALPHA, N_EXPERIENCES // 6)
        for exp in range(N_EXPERIENCES // 6):
            self.train_first_player(env, player1, player3, exp, verbose, ALPHAS[exp])
        # train agent 1 to defense the random agent
        for exp in range(N_EXPERIENCES // 6, 2 * N_EXPERIENCES // 6):
            self.train_second_player(env, player3, player1, exp, verbose, ALPHAS[exp])
        # train agent 2 to attack the random agent
        for exp in range(2 * N_EXPERIENCES // 6, 3 * N_EXPERIENCES // 6):
            self.train_first_player(env, player2, player3, exp, verbose, ALPHAS[exp])
        # train agent 2 to defense the random agent
        for exp in range(3 * N_EXPERIENCES // 6, 4 * N_EXPERIENCES // 6):
            self.train_second_player(env, player3, player2, exp, verbose, ALPHAS[exp])
        #player2 = player1.deep_copy()
        # train agent 1 to attack the agent 2 & agent 2 to defense agent 1
        for exp in range(4 * N_EXPERIENCES // 6, 5 * N_EXPERIENCES // 6):
            self.train_both_player(env, player1, player2, exp, verbose, ALPHAS[exp])
        # train agent 2 to attack the agent 1 & agent 1 to defense agent 2
        for exp in range(5 * N_EXPERIENCES // 6, N_EXPERIENCES):
            self.train_both_player(env, player2, player1, exp, verbose, ALPHAS[exp])
